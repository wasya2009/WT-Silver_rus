...
init -1 python hide:v
    ...
    config.window_title = u"Тренер Ведьмы v1.31rus_rev0.3 (Серебрянное издание) MoCoder и Red Machine teams"

    # These control the name and version of the game, that are reported
    # with tracebacks and other debugging logs.
    config.name = "WT Silver"
    config.version = "1.31_3"
...
python early:
    config.save_directory = "WT SILVER(rus)"
...
init python:
    ...
    build.directory_name = "WT_Silver_1.31-rus_rev0.3"

    ## The name that's uses for executables - the program that users will run
    ## to start the game. For example, if this is 'mygame', then on Windows,
    ## users can click 'mygame.exe' to start the game.
    build.executable_name = "Witch Trainer(Silver)"
    ...
    build.classify('game/**.aif', 'sounds')
    ...
    layout.DELETE_SAVE = "Вы уверены, что хотите удалить сохранение?" 
    layout.OVERWRITE_SAVE = "Вы уверены, что хотите перезаписать сохранение?" 
    layout.ARE_YOU_SURE = "Вы уверены?" 
    layout.LOADING = "Вы уверены, что хотите загрузить игру?" 
    layout.QUIT = "Шалость удалась?" 
